...
// public static string ProjectDir => Directory.GetCurrentDirectory().ToString();
//public static string ProjectDir => Directory.GetParent(Application.dataPath).ToString();
/*        string targetDir = $"{ProjectDir}/Bash";
        Process p = new Process();
        p.StartInfo.WorkingDirectory = targetDir;
        p.StartInfo.FileName = "Libil2cpp.bat";
        *//*ProcessStartInfo pi = new ProcessStartInfo(p.StartInfo.FileName, parm);//第二个参数为传入的参数，string类型以空格分隔各个参数  
...
/* string targetDir = $"{ProjectDir}/BasicBash";
 Process p = new Process();
 *//*p.StartInfo.WorkingDirectory = targetDir;
 p.StartInfo.FileName = $"{targetDir}/BuildAndroid.bat";*//*
 ...
 pi.UseShellExecute = false;
 pi.RedirectStandardOutput = true;
 pi.FileName= $"{targetDir}/BuildWin.bat";
 pi.WorkingDirectory = targetDir;
 p.StartInfo = pi;   
 p.Start();
 p.WaitForExit();*/
/*p.StartInfo.Arguments = parm1;
 p.StartInfo.UseShellExecute = false;
p.StartInfo.CreateNoWindow = false;
p.Start();
p.WaitForExit();*/
...
// GetGit("2020.3.33");
// Local_il2cpp(@"E:\unity\2020.3.33f1c2\Editor\Data\il2cpp");
// BuildTest(@"E:\unity\2020.3.33f1c2\Editor\Unity.exe", @"F:\hyclr\batTest\hybridclr_test");